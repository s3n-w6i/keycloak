...
impl<TS: KeycloakTokenSupplier> KeycloakAdmin<TS> {
    ...
    /// `GET /admin/realms`
    ///
    /// Documentation: <https://www.keycloak.org/docs-api/26.5.5/rest-api/index.html#_get_adminrealms>
    pub async fn get(
        &self,
    ...
    /// `POST /admin/realms`
    ///
    /// Documentation: <https://www.keycloak.org/docs-api/26.5.5/rest-api/index.html#_post_adminrealms>
    pub async fn post(
        &self,
    ...
    pub async fn realm_admin_events_get(
        ...
        if let Some(v) = operation_types {
            builder = builder.query(&v.into_iter().map(|e| ("operationTypes", e)).collect::<Vec<_>>());
        }
        if let Some(v) = resource_path {
            builder = builder.query(&[("resourcePath", v)]);
        }
        if let Some(v) = resource_types {
            builder = builder.query(&v.into_iter().map(|e| ("resourceTypes", e)).collect::<Vec<_>>());
        }
        let response = builder.send().await?;
        Ok(error_check(response).await?.json().await?)
    }
